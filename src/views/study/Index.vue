<template>
  <div class="page">
    ...
    <div class="study container">
      ...
      <div class="study-nav">
        <div class="nav-list" @click="tab(index)" v-for="(item,index) in nav" :class="{active : index===curId}" :key="index">
  ...
  data(){
    ...
    mounted:function(){
     console.log(this.id,this.tit);
      this.$http.post('http://127.0.0.1:3000/thiskclistdetail',
      `kcname=${this.tit}&id=${this.id}`).then((res)=>{
      ...
      // 获取课程评价
      this.$http.post('http://127.0.0.1:3000/demo/kcpj',
      `kcid=${this.id}`).then((res)=>{
          if(res.data.code){
            this.pinjia = []
          }else{
       ...
       // 获取当前用户头像与积分
      this.$http.post('http://127.0.0.1:3000/demo/getusermsg',
      `username=${this.$store.state.username}`).then((res)=>{
          if(res.data.code){
            this.userimg = ''
          }else{
      ...
      // 获取是否订阅
       this.$http.post('http://127.0.0.1:3000/demo/isdinyue',
      `username=${this.$store.state.username}&kcid=${this.id}`).then((res)=>{
    ...
    methods:{
      ...
      guanzhu(item){
        ...
        this.$http.post('http://127.0.0.1:3000/demo/addmyguanzhu',
        `username=${this.$store.state.username}&kcid=${this.id}`).then((res)=>{
             alert(res.data.msg)
          }).catch((err)=>{
              alert(err);
      ...
      if(this.userjf<this.kcdetail.kcjf){
        ...
        this.$http.post('http://127.0.0.1:3000/demo/addmydinyue',
      `username=${this.$store.state.username}&kcid=${this.id}&userjf=${userjf}`).then((res)=>{
      ...
      addpj(){
        ...
        console.log(pjtime);
        this.$http.post('http://127.0.0.1:3000/demo/addkcpj',
        `pjtime=${pjtime}&userimg=${this.userimg}&kcpjmsg=${this.kcpjmsg}&username=${this.$store.state.username}&kcid=${this.id}`).then((res)=>{
           console.log(res);
            this.pinjia = res.data;
