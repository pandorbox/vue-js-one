<template>
  <div class="page container">
    ...
    <div class="user">
      ...
      <div class="user-nav">
        <div class="nav3-list" @click="tab(index)" v-for="(item,index) in nav" :class="{active : index===curId}" :key="index">
      ...
      <div v-if="curId == 4" class="nav3-msg">
          ...
          <div class="nav3-nootbook"  v-for="(item, index) in bjturn" :key="index" >
            <div class="nootbook-tit">
              {{item.bjtit}}
            </div>
            <div class="nootbook-msg">
              {{item.bjmsg}}
            </div>
            <div class="nootbook-time">
              {{item.bjtime}}
            </div>
          </div>
      </div>
...
export default {
  ...
  data(){
    ...
    console.log("username",this.$store.state.username)
      ...
      // 获取个人笔记
        this.$http.post('http://127.0.0.1:3000/demo/mybj',
         `username=${this.$store.state.username}`
         ).then((res)=>{
       ...
       // 获取关注列表
        this.$http.post('http://127.0.0.1:3000/demo/myguanzhu',
         `username=${this.$store.state.username}`
         ).then((res)=>{
             var kclist= res.data;
           ...
           // 获取订阅列表
        this.$http.post('http://127.0.0.1:3000/demo/mydinyue',
         `username=${this.$store.state.username}`
         ).then((res)=>{
             var kclist= res.data;
             for(var i=0;i<kclist.length;i++){
      ...
      this.$http.post('http://127.0.0.1:3000/thiskclistdetail',
      `kcclass=${this.tit}&id=${this.id}`).then((res)=>{
      ...
      adduserbj(){
        ...
        console.log(this.$store.state.username)
          this.$http.post('http://127.0.0.1:3000/demo/addmybj',
