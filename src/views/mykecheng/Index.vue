<template>
  <div class="page container">
    ...
    <div class="user">
      ...
      <div class="user-nav">
        <div class="nav2-list" @click="tab(index)" v-for="(item,index) in nav" :class="{active : index===curId}" :key="index">
            <div :class="{active : index===curId}" class="nav2-list-name">{{item}}</div>
        </div>
      </div>
      <div v-if="curId == 0" class="nav2-msg">
          <div class="nav2-msg-list"  v-for="(item, index) in mykc" :key="index" >
      ...
      <div v-if="curId == 1" class="addkecheng">
            <form  action="">
              <div class="zuozhe">
                ...
                <input class="input-lang" type="text" disabled="true"  :value="this.$store.state.username" name="kcauthor" id="inputkcauthor">
              </div>
      ...
      <div v-if="curId == 2" class="nav2-msg">
          <div class="nav2-usermsg-list"  v-for="(item, index) in pinjia" :key="index" >
...
export default {
  ...
  data(){
    return{
      ...
      mykc:[],
pinjia: [
         {img:'@/assets/img/header/logo.png',username:'张三',msg:'张三',time:'2019-05-13',zan:2},
         {img:'@/assets/img/header/logo.png',username:'张三',msg:'张三',time:'2019-05-13',zan:2},
         {img:'@/assets/img/header/logo.png',username:'张三',msg:'张三',time:'2019-05-13',zan:2},
         {img:'@/assets/img/header/logo.png',username:'张三',msg:'张三',time:'2019-05-13',zan:2},
         {img:'@/assets/img/header/logo.png',username:'张三',msg:'张三',time:'2019-05-13',zan:2}
    ...
    mounted:function(){
      ...
      this.$http.post('http://127.0.0.1:3000/demo/mymsg',
      ...
      // 获取我的课程
      this.$http.post('http://127.0.0.1:3000/demo/mykc',
         `username=${this.$store.state.username}`
         ).then((res)=>{
           if(res.data.code==401){
             console.log(res);
    ...
    methods:{
      ...
      kcaudio(){
         ...
         data.append("kcjf",inputkcjf.value);
          ...
          for(let item of audiofiles){data.append('kcaudio',item);
          }
         this.$http.post('http://127.0.0.1:3000/demo/addkclistone',data).then((res)=>{
      ...
      addlist(){
         ...
         data.append("kcgs",inputkcgs.value); 
          ...
          for(let item of imgfiles){data.append('kcimg',item);
          }
          console.log(data)
         this.$http.post('http://127.0.0.1:3000/demo/addkclist',data).then((res)=>{
             alert( JSON.stringify(res.data));
          }).catch((err)=>{
              alert(err);
          });
      ...
      userimg(){
       ...
       for(let item of files){data.append('userimg',item);}
       var username=this.$store.state.username
       data.append('username',username);
       console.log(data);
       this.$http.post('http://127.0.0.1:3000/demo/adduserimg',data).then((res)=>{
             this.usermsg = res.data;
          }).catch((err)=>{
              alert(err);
          });
      ...
      addusermsg(){
        //  var data=new FormData();
        //  data.append('kcimg',this.kcimg);
        console.log(this.$store.state.username)
          this.$http.post('http://127.0.0.1:3000/demo/addmymsg',
         `kcimg=${this.kcimg}&username=${this.$store.state.username}&usernc=${this.usernc}&userjj=${this.userjj}`
         ).then((res)=>{
           console.log(res)
             this.usermsg=res.data;
             this.thistc = false;
